...
2018-10-17   | Enhance WebVOWL and Protégé documentation (KG)
...
### Browse Ontologies with Protégé
Protégé is an open source tool where you can develop and manage OWLs, so define the data structure and concept for the data linkage. It is used in this project by some members to create and maintain the OWL.

![Figure: Screenshot of Protégé](./doc/images/protege_overview.PNG)

On the picture you see the study.ttl file. The left pane displays the class hierarchy. Everything is a subclass of "Thing". What ever is in bold, this is defined in the currently opened owl file (so in study.ttl). All other items come from other referenced owl files. What you can see in the screenshot is that "Enrolled subject" is a subclass from "Human study subject" which is a subclass from "Human subject" and so on. This class hierarchy does not dipslay other relationships, so to investigate the connections using Protégé is a bit difficult.
...
To build an OWL using Protégé is the typical process. To figure out the structure when you have an available OWL is difficult with this tool. If you have any tips, please add them or let the the project members know, so they can add this.
...
```
stardog namespace add CTDasRDFSMS --prefix cd01p --uri https://raw.githubusercontent.com/phuse-org/CTDasRDF/master/data/rdf/cdiscpilot01-protocol.ttl#
stardog namespace add CTDasRDFSMS --prefix cdiscpilot01 --uri https://raw.githubusercontent.com/phuse-org/CTDasRDF/master/data/rdf/cdiscpilot01.ttl#
stardog namespace add CTDasRDFSMS --prefix code --uri https://raw.githubusercontent.com/phuse-org/CTDasRDF/master/data/rdf/code.ttl#
stardog namespace add CTDasRDFSMS --prefix sdtm --uri https://raw.githubusercontent.com/phuse-org/CTDasRDF/master/data/rdf/sdtm.ttl#
stardog namespace add CTDasRDFSMS --prefix sdtmterm --uri https://raw.githubusercontent.com/phuse-org/CTDasRDF/master/data/rdf/sdtm-terminology.rdf#
stardog namespace add CTDasRDFSMS --prefix study --uri https://raw.githubusercontent.com/phuse-org/CTDasRDF/master/data/rdf/study.ttl#
```
